...
import { FaAngleLeft, FaAngleRight } from "react-icons/fa";   // ← Added
...
const DailyRoiIncome = () => {
  ...
  const { 
    data: apiResponse = { history: [], totalPages: 0, totalRecords: 0 }, 
    isFetching, 
    isError 
  } = useQuery({
    queryKey: ["perDayIncome", pagination.pageIndex, userIdFilter, startDate, endDate],
    queryFn: async () => {
      const pageNum = pagination.pageIndex + 1;
      const res = await adminApi.getPerDayIncomeReport(pageNum, 10, startDate, endDate, userIdFilter);
      console.log(`📡 Fetched Page ${pageNum} → ${res?.data?.history?.length || 0} records`);
      return res?.data || { history: [], totalPages: 0, totalRecords: 0 };
    },
    refetchOnWindowFocus: false,
    staleTime: 0,
  });

  const roiDistributions = apiResponse.history ?? [];
  ...
  const fetchFullData = async () => {
    ...
    while (page <= totalPages) {
      const res = await adminApi.getPerDayIncomeReport(page, 500, startDate, endDate, userIdFilter);
      const data = res?.data || {};
    ...
    try {
      ...
      for (let i = 0; i < allData.length; i += chunkSize) {
        ...
        autoTable(doc, {
          ...
          body: rows,
          startY: i === 0 ? 20 : doc.lastAutoTable?.finalY || 20,
          styles: { fontSize: 8, cellPadding: 2 },
          headStyles: { fillColor: [16, 57, 68], textColor: [255, 255, 255] },
        });
      }
      doc.save("daily-roi-income.pdf");
    } catch (err) {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, "daily-roi-income.xlsx");
    } catch (err) {
  ...
  return (
    <div className="p-4 max-w-[1260px] mx-auto">
      <h2 className="text-2xl font-bold mb-4 text-[#103944]">Daily ROI Income</h2>
      ...
      <div className="relative overflow-x-auto bg-white rounded-lg shadow border min-h-[400px]">
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {roiDistributions.length === 0 && !isFetching ? (
      ...
      {/* ✅ Your Requested Pagination (First | Prev | Next | Last) */}
