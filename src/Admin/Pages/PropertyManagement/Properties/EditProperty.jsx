...
export default function EditInvestmentPlan() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    setData({
      ...
      image: p.image ? { preview: p.image, existing: true } : null,
  ...
  const updatePlanMutation = useMutation({
    mutationFn: (payload) => adminApi.updatePlan(id, payload),
    onSuccess: () => {
  ...
  const uploadToPinata = async (file) => {
    if (!file || file.existing) return typeof data.image === "string" ? data.image : null;
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex justify-end gap-4">
          ...
          <button
            ...
            disabled={updatePlanMutation.isLoading || !isFormValid()}
