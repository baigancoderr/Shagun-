  ...
  return (
    <div className="theme-card-style border-gradient text-gray-800 p-6 rounded-md max-w-full mx-auto">
      <div className="flex justify-between mb-6 gap-4 flex-wrap-reverse">
        <h2 className="text-2xl text-[#FF6000] font-bold">Withdraw Report</h2>
        <button
          onClick={exportToExcel}
          className="px-3 py-1 h-fit text-base border flex items-center justify-center gap-2 border-slate-600 rounded bg-transparent hover:bg-[#ff6000] transition"
        >
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <input
          ...
          className="flex-1 px-4 py-2 bg-transparent border border-gray-500 rounded text-white focus:outline-none focus:border-[#FF6000] focus:ring-1 focus:ring-[#FF6000]"
        />
    <select
  value={statusFilter}
  onChange={(e) => setStatusFilter(e.target.value)}
  className="px-4 py-2 bg-black text-white border border-gray-700 rounded focus:outline-none focus:border-[#FF6000] focus:ring-1 focus:ring-[#FF6000]"
>
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  <tr key={headerGroup.id}>
                    {headerGroup.headers.map((header) => (
                      <th
                        key={header.id}
                        className="text-left px-4 py-2 border-b border-slate-700 text-[#FF6000] text-nowrap"
                      >
                        {flexRender(
