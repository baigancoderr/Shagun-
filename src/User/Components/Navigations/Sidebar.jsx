...
import logo1 from "../../../assets/userImages/Logo/logo_lght1.png";
...
const navItems = [
...

  {
    ...
    name: "Invest Package",
    subItems: [
      { name: "Package Investments", path: "property-investments", pro: false },
      { name: "Package Report", path: "property-investment-report", pro: false },
    ],
  },

  {
    ...
    name: "Shopping",
    subItems: [
      { name: "Shopping Center", path: "swap", pro: false },
      // { name: "Swap", path: "usd-to-token-swap", pro: false },
      { name: " Shopping Report", path: "swap-report", pro: false },
    ],
  },
  ...
  // },
  {
    ...
    subItems: [
      ...
      // { name: "Binary Income", path: "binary-income-report", pro: false },
      // { name: "Leadership Share", path: "leadership-share-report", pro: false },
      //  { name: "Leadership Income", path: "leadership-income-report", pro: false },
    ],
  },
   {
    icon: <TbReportMoney />,
    name: "KYC",
    subItems: [
      { name: "Submit KYC", path: "kyc-submit", pro: false },
      { name: "KYC Report", path: "kyc-submit-report", pro: false },
    ],
  },
  {
    icon: <FaMoneyBillWave />,
  ...
  // { icon: <FaUserCircle />, name: "Tokenized Agreement", path: "tokenized-agreement" },
...
const Sidebar = () => {
            ...
            {nav.subItems ? (
              <button
                ...
                ${openSubmenu?.index === index
                    ? "bg-gradient-to-r from-[#FF6000] to-[#E3090D] text-[#fff] shadow-sm border border-orange-100"
                    : "text-white hover:bg-[#FF6000] hover:text-[#000]"
                  } 
                ${!isExpanded && !isHovered ? " lg:px-[0.9rem]" : " "} lg:justify-start`}
              >
                <div className="absolute inset-0 bg-gradient-to-r from-[#FF6000] to-[#E3090D] opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-xl" />
                <span className={`relative z-10 flex-shrink-0 text-lg transition-colors duration-200
                ${openSubmenu?.index === index
                    ? "text-[#fff]"
                    : "text-[#FF6000] group-hover:text-white"
                  }`}>
                  {nav.icon}
                </span>
                {(isExpanded || isHovered || isMobileOpen) && (
              ...
              (nav.target === "_blank" ? (
                <a
                  ...
                  ${isActive(nav.path)
                      ? "bg-gradient-to-r from-[#FF6000] to-[#E3090D] text-white shadow-sm border border-orange"
                      : "text-white hover:bg-[#FF6000] hover:text-white"
                    }
                  ${!isExpanded && !isHovered ? "lg:px-[0.9rem]" : " "}lg:justify-start`}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-[#FF6000] to-[#E3090D] opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-xl" />
                  ...
                  ${isActive(nav.path)
                      ? "text-white"
                      : "text-[#FF6000] group-hover:text-[#fff]"
                    }`}>
                    {nav.icon}
                  </span>
                  {(isExpanded || isHovered || isMobileOpen) && (
                    ...
                    <li key={subItem.name}>
                      <Link
                        ...
                        ${isActive(subItem.path)
                            ? "bg-gradient-to-r from-[#FF6000] to-[#E3090D] text-white shadow-sm"
                            : "text-white hover:bg-[#FF6000] hover:text-white"
                          }`}
                      >
                        <div className="absolute inset-0 bg-gradient-to-r from-[#FF6000] to-[#E3090D] opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg" />
                        ...
                        {(subItem.new || subItem.pro) && (
                          <span className="relative z-10 flex gap-1.5 ml-2">
                            ...
                            {subItem.pro && (
                              <span className="bg-gradient-to-r from-[#FF6000] to-[#E3090D] text-white text-sm font-semibold px-2 py-0.5 rounded-full shadow-sm">
                                PRO
                              </span>
                            )}
                          </span>
  ...
  return (
    <aside
      className={`fixed top-0 z-[99999] left-0 h-[100vh] bg-black backdrop-blur-md border-r border-[#FF6000] transition-all duration-300 ease-in-out shadow-lg
      ...
      {isMobileOpen ?
        <>
          <div
            ...
            className="absolute bg-[#FF6000] px-2 right-[-10px] top-5 rounded-md text-white cursor-pointer hover:bg-black-400">
            ✕
          </div>
        </> :
        <></>}
      {/* LOGO SECTION */}
      <div className={`py-6 px-6 flex border-b border-[#FF6000] ${!isExpanded && !isHovered ? "lg:px-[0.9rem]" : ""} justify-start`}>
      ...
      <div className="flex flex-col h-full pb-36 overflow-y-auto no-scrollbar">
        <nav className="flex-1 px-3 py-6">
          <div className="space-y-8">
            <div>
              <h2 className={`mb-6 text-sm font-bold uppercase tracking-wider flex leading-5 ${!isExpanded && !isHovered ? "lg:justify-center text-[#fff]" : "justify-start text-[#ffffff]"}`}>
                {isExpanded || isHovered || isMobileOpen ? "Navigation" : <div className="w-6 h-0.5 mb-3 bg-[#fff] font-[300] rounded-full" />}
              </h2>
              {renderMenuItems()}
            </div>
          </div>
